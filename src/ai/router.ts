...
export async function processMessage(input: RouterInput): Promise<RouterResult> {
	...
	try {
		...
		handleAiError(error, provider, modelName);
	}
}

/**
 * Handles AI-related errors by logging them and throwing user-friendly error messages.
 */
function handleAiError(error: unknown, provider: Provider, modelName: string): never {
